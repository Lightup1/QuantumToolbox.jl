...
- `kwargs`: The keyword arguments for the solver. When SteadyStateLinearSolver() is used, you can pass u0 in kwargs as a inital guess for linear solver. u0 should be a vector corresponding to the density matrix as, for example, `u0=vec(Matrix(ρ.data))`. When SteadyStateODESolver() is used, you can pass any keyword arguments for `mesolve` in `kwargs...`, but the `progress_bar`, `save_everystep`, and `saveat` will be ignored and overwritten.
"""
